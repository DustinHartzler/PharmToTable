...
//    ThriveApp.checkHeight();
    ...
    ThriveApp.post_title_background();
...
//        ThriveApp.checkHeight();
    ...
    //focus areas
    jQuery('.far.f1').on('click', function (e) {
        jQuery(this).addClass('fof');
        e.preventDefault();
    });

    //bold first pagraph
    jQuery('.hfp').children().each(function () {
...
ThriveApp.post_title_background = function () {
    var _title_wrapper = jQuery('.hru.fih');
    _title_wrapper.css('max-height', ThriveApp.viewportHeight);
}
...
//        } else {
//            var _parentWidth = isFullWidth ? ThriveApp.winWidth : img.parent().width(),
//                ratio = (
...
//        SUPP-2813 & SUPP-2776 conflict fix
...
ThriveApp.menuResponsive = function () {
    if (ThriveApp.winWidth <= 768) {
        ...
        jQuery('#nav_right').on('click', function () {
            jQuery(this).css('display', 'block');
        });

        var navigationElement = jQuery('#nav_right ul li');
...
ThriveApp.truncatePostGallery = function () {
    var $targets = jQuery('.gdl .text-truncate');
    if ($targets.length === 0) {
        return;
    }
...
var _lastScrollTop = 0,
    ...
    _hero = jQuery('.hru'),
    _hasHero = _hero.length,
    _heroHeight = _hasHero ? _hero.height() : 60,
...
ThriveApp.menu_float = {
    ...
    hasScroll: function () {
        if (_is_custom_header) {
            ...
            return this.current_scroll_top > _heroHeight;
        }
    },
    ...
    handle: function () {
        ...
        if (this.current_scroll_top > _heroHeight) {
...
jQuery(window).scroll(function () {
    ...
    if (hasTitle == 1) {
        ...
        condition = position > containerTopPosition;
    }
    if (condition) {
        ...
        if (position > containerTopPosition + containerHeight - socialElement.height()) {
            ...
            if (jQuery('.bSe').hasClass('right')) {
                _this.css({'right': '-82px', 'left': 'auto'});
            } else {
                _this.css({'left': '-82px', 'right': 'auto'});
            }
            ;
        } else {
            ...
            if (jQuery('.bSe').hasClass('right')) {
                ...
                _this.css('left', socialPosition + 10 + 'px');
            } else {
                socialPosition = jQuery('.bSe').offset().left;
                _this.css('left', socialPosition - _this.width() - '10');
            }
            ;
        }
    } else {
...
ThriveApp.post_format_gallery = function () {
    jQuery("#thrive-gallery-header").attr("data-count", jQuery(".thrive-gallery-item").length);

    jQuery(".thrive-gallery-item").click(function () {
        jQuery("#thrive-gallery-header").attr("data-index", jQuery(this).attr("data-index"));
        jQuery("#thrive-gallery-header").css('background-image', 'url(' + jQuery(this).attr("data-image") + ')');
        return false;
    });

    jQuery(".gprev").click(function () {
        var _currentIndex = parseInt(jQuery("#thrive-gallery-header").attr("data-index"));
        var _prevIndex = _currentIndex - 1;
        if (_prevIndex < 0) {
            _prevIndex = jQuery("#thrive-gallery-header").attr("data-count") - 1;
        }
        if (jQuery("#li-thrive-gallery-item-" + _prevIndex).length > 0) {
            jQuery("#thrive-gallery-header").attr("data-index", jQuery("#li-thrive-gallery-item-" + _prevIndex).find("a").attr("data-index"));
            jQuery("#thrive-gallery-header").css('background-image', 'url(' + jQuery("#li-thrive-gallery-item-" + _prevIndex).find("a").attr("data-image") + ')');
            jQuery("#thive-gallery-dummy").attr("src", jQuery("#li-thrive-gallery-item-" + _prevIndex).find("a").attr("data-image"));
        }
        return false;
    });
    ...
    jQuery(".gnext").click(function () {
        ...
        if (_nextIndex >= jQuery("#thrive-gallery-header").attr("data-count")) {
            _nextIndex = 0;
        }
        if (jQuery("#li-thrive-gallery-item-" + _nextIndex).length > 0) {
            jQuery("#thrive-gallery-header").attr("data-index", jQuery("#li-thrive-gallery-item-" + _nextIndex).find("a").attr("data-index"));
            jQuery("#thrive-gallery-header").css('background-image', 'url(' + jQuery("#li-thrive-gallery-item-" + _nextIndex).find("a").attr("data-image") + ')');
            jQuery("#thive-gallery-dummy").attr("src", jQuery("#li-thrive-gallery-item-" + _nextIndex).find("a").attr("data-image"));
        }
        return false;
    });
