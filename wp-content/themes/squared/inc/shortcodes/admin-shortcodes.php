...
function thrive_print_shortcodes() {
	...
	<script type="text/javascript">
		...
		var thrive_shortcodes = [
			...
			'Conversion',
			'Button', 'SplitButton', 'MegaButton', 'Countdown', 'Price', 'Testimonal', 'CustomGrid', 'Optin',
			'NumbersAndData',
		...
		/*var thrive_shortcodes = ['Button', 'BlankSpace', 'ContentContainer', 'ContentBox', 'CustomMenu', 'CustomFont', 'Borderless', 'Code', 'DividerLine', 'FollowMe', 'Phone',
		 'GMaps', 'Grid', 'Tabs', 'Toggle', 'PageSection', 'Pullquote', 'PostsList', 'PostsGallery', 'Price', 'Optin', 'Testimonal', 'VideoSection', 'ResponsiveVideo'];*/
		var thrive_shortcodes2 = [
			'Halves', 'Columns-1-2', 'Columns-1-3', 'Thirds', 'Columns-2-3-1', 'Columns-3-2-1', 'Fourths', 'Columns-1-4', 'Columns-3-4-1',
...
function thrive_shortcode_display_options() {
	...
	switch ( $_GET['sc'] ) {
		...
		case 'Headline':
			$all_colors = _thrive_get_color_scheme_options( "buttons" );
			$req_path   = get_template_directory() . "/inc/shortcodes/admin-headline-options.php";
			break;
		case 'ContentBox':
		...
		case 'CustomGrid':
			$img_sizes = array(
				'small'  => __( "Small", 'thrive' ),
				'medium' => __( "Medium", 'thrive' ),
				'large'  => __( "Large", 'thrive' )
			);
...
function thrive_add_quicktags() {
	...
	if ( ! $isTvePage ) {
		...
		if ( ( ! $post || ( $screen->base != "post" ) ) && ( $screen->id != "thrive-dashboard_page_thrive_admin_options" ) ) {
			return;
		}
	}
...
function _thrive_get_vimeo_embed_code( $url, $width = "100%" ) {
	...
	if ( ! preg_match( '#vimeo\.com/(\w*/)*(\d+)#', $url, $m ) ) {
		return '';
	}
...
function _thrive_generate_price_shortcode_output( $attr, $content, $type, $position ) {
	...
	$output .= "<div class='" . $in_div_class . "'><h5 class='ctr'>" . $attr['title'] . "</h5><ul class='check'>";
...
function thrivev2_add_my_tc_button( $hook ) {
	...
	if ( ! $isTvePage ) {
		$screen = get_current_screen();
		if ( ( 'post.php' != $hook && 'post-new.php' != $hook ) && $screen->id != "thrive-dashboard_page_thrive_admin_options" ) {
			return;
		}
...
function thrive_add_tinymce( $hook ) {
	$screen = get_current_screen();
	global $isTvePage;
	if ( ( 'post.php' != $hook && 'post-new.php' != $hook ) && $screen->id != "thrive-dashboard_page_thrive_admin_options" && ! $isTvePage ) {
		return;
	}
