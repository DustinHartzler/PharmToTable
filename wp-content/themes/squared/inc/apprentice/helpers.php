...
function _thrive_get_next_prev_lesson_link( $lessonId, $next = true ) {
	...
	foreach ( $lessonsOrder as $catId => $lessonsIds ) {
		...
		if ( count( $lessonsIds ) === 1 && isset( $lessonsIds['order'] ) ) {
			$order .= $lessonsIds['order'];
			$index = $order;
			continue;
		}

		if ( count( $lessonsIds ) > 1 ) {
			$lessonAdded = true;
			$i           = $index . $lessonsIds['order'];

			unset( $lessonsIds['order'] );

			$newOrder[ $i ] = $lessonsIds;
		}
	}
	...
	$all_lessons = array();
	foreach ( $newOrder as $lessonsIds ) {
		$all_lessons = array_merge( $all_lessons, $lessonsIds );
	}
