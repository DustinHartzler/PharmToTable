					...
					<div class="<?php echo $comment_container_class; ?>">
						...
						<div class="ccr right">
							...
							<?php if ( comments_open() && ! post_password_required() && '0' != $comment->comment_approved ) : ?>
								<a class="rpl reply right" href="#" id="link-reply-<?php echo get_comment_ID(); ?>"
...
function thrive_enqueue_head_fonts() {
	...
	if ( is_array( $post_id ) ) {
		foreach ( $post_id as $id ) {
			$post_fonts = get_post_meta( $id, 'thrive_post_fonts', true );
			if ( ! empty( $post_fonts ) ) {
				$fonts = json_decode( $post_fonts );
				if ( $fonts != null ) {
		...
		$post_fonts = get_post_meta( $post_id, 'thrive_post_fonts', true );
		if ( ! empty( $post_fonts ) ) {
			$fonts = json_decode( $post_fonts );
			if ( $fonts != null ) {
				foreach ( $fonts as $key => $font ) {
...
function thrive_theme_comment_nav() {
	...
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) {
		echo "<div class='ctr pgn clearfix'>";
